...
IDA only scans `loaders/` directories for loader modules, which is why the symlink exists at all. `install_loader_links` creates a symlink from `$IDAUSR/loaders/vtloader_loader.py` to the entry file in the plugin root. Windows has supported non-admin symlinks since the Creators Update (2017), which is well within IDA 9.2's system requirements. A symlink makes the relationship between the loader and the plugin visible: if the plugin is uninstalled, the symlink becomes dangling, and IDA skips the missing file.

`remove_retired_links` runs first and deletes any other loader-named file that is ours, under either the `vtloader_` prefix or the `memloader_` one this plugin used up to version 1.1: that plugin's ZIP, URL and VirusTotal links, links left behind when the plugin root moves, dangling symlinks, generated stub files, and hard links from pre-1.1 vtloader on Windows. Ours is recognised by the symlink target or by the first line of the file, which is the entry file docstring under either name or the old stub marker. A live link into some other plugin's directory is left alone, as is any file that is not ours.

Neither the plugin nor the entry file modifies `sys.path`. IDA adds every plugin directory that holds an `ida-plugin.json` to `sys.path` when it loads plugins, so `vtloader` imports as a regular package from both. The entry file imports the loader module and defines `accept_file` and `load_file` wrappers that forward the call. When the import raises `ModuleNotFoundError` for `vtloader`, `accept_file` returns 0. Other import errors propagate.
...
`loader.accept_file` returns 0 in batch mode when no `sha256` option is present, and the format name otherwise, so an ordinary file still loads normally when the loader was not asked for. It also returns 0 while `core` is offering a downloaded buffer to the loader list: a loader that accepts any input is otherwise its own best candidate for its own download, and the load recurses instead of falling back to shellcode. `load_file` obtains the hash (option or `ask_str`), downloads it with the key from the plugin settings, and calls `load_buffer_into_ida` under the hash name with the Downloads directory.
...
`tests/conftest.py` locates an IDA installation from `IDADIR` or `~/.idapro/ida-config.json` (appending `Contents/MacOS` for a macOS app bundle), creates a temporary `IDAUSR` with the license files and a `plugins/vtloader` symlink to the plugin directory, and exports both variables before `idapro` is imported. The plugin then runs at `import idapro` and creates the loader symlink itself. One test starts a separate idalib process with only these variables set, which proves that the installed plugin alone makes the loader importable. The `ida` fixture imports `idapro`, accepts the EULA through the registry, and skips when no installation is available or the IDA SDK version is older than 9.2; idalib in 9.1 exits the process on the `-T` switch and idalib in 9.0 ignores `-O` options. The unit tests run on Linux, macOS, and Windows to confirm symlink creation works across platforms.
...
Nothing in the suite stubs or replaces part of the plugin. The tests that must not touch the network call `core.load_buffer_into_ida` on a database that idalib has already opened, which is where the loader calls it from too. Two of them open that database from a PE with a `sha256` option set: `accept_file` then claims any input, so the pipeline has to decline the buffer it is loading itself, and the PE occupies 0x401000, which leaves address 0 free for the shellcode fallback. `tests/pe.py` builds a 1 KiB PE32 with one `.text` section that IDA's PE loader accepts; `tests/data/pma-lab01-01.zip` supplies real sample bytes for the live download test.
