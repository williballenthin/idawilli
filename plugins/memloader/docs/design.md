...
memloader_entry.py       PLUGIN_FIX plugin: keeps the loader link current, VirusTotal menu entry
loaders/
  memloader_vt_loader.py Loader entry file that $IDAUSR/loaders/ links to
memloader/
  ...
  vt_loader.py           accept_file / load_file
...
`remove_retired_links` runs first and deletes every other `memloader_*.py` that is ours: the ZIP and URL loader links that versions before 2.0 installed, links left behind when the plugin root moves, dangling symlinks, and generated stub files from earlier versions. Ours is recognised by the symlink target, by `samefile`, or by the first line of the file, which is either the entry file docstring or the old stub marker. Any other `memloader_`-named file is left alone. `remove_loader_links` deletes only links into a given plugin root.
...
5. A best candidate with the archive flag is rejected: Memloader cannot open archives in memory.
