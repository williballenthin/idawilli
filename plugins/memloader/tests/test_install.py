...
RETIRED = ("memloader_zip_loader.py", "memloader_url_loader.py")
...
def make_plugin_root(tmp_path: Path, marker: str = "TEST") -> Path:
    ...
    get_link_target(root).write_text(
        f'"""IDA loader entry for Memloader {marker}."""\n'
    )
    return root
...
def test_install_retires_the_zip_and_url_loader_links(tmp_path):
    """Upgrading from a version with three loaders must not leave the other two behind."""
    loaders = tmp_path / "loaders"
    loaders.mkdir()
    plugin_root = make_plugin_root(tmp_path)
    for name in RETIRED:
...
def test_install_removes_dangling_links_but_leaves_foreign_files(tmp_path):
    ...
    dangling.symlink_to(tmp_path / "elsewhere" / "memloader_removed_loader.py")
...
def test_install_refuses_to_replace_a_file_it_did_not_create(tmp_path):
    ...
    with pytest.raises(OSError, match="not created by Memloader"):
...
def test_plugin_alone_makes_ida_find_the_loader(ida, tmp_path):
    ...
    result = run_idalib(tmp_path, Path(os.environ["IDAUSR"]), sample, "-Omemloader:")
