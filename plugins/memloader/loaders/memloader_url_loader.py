"""IDA loader entry for Memloader URL. IDA reaches it through a link in ``$IDAUSR/loaders/``.
...
url_loader: types.ModuleType | None
try:
    from memloader.loaders import url_loader as _module
    ...
    ida_kernwin.msg(
        ...
        f"Delete {ida_diskio.get_user_idadir()}/loaders/memloader_url_loader.py to remove this message.\n"
    )
else:
    url_loader = _module
...
def accept_file(li, filename):
    if url_loader is None:
        return 0
    return url_loader.accept_file(li, filename)


def load_file(li, neflags, format):
    if url_loader is None:
        raise RuntimeError("Memloader plugin is not installed")
    return url_loader.load_file(li, neflags, format)
