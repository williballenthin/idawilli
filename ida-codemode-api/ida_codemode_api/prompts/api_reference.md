...
| `get_segment_containing(address)` | `{name: str, start: int, end: int, size: int, permissions: int, class: str, bitness: int}` | Segment descriptor for the segment containing an address. |
...
| `read_pointer(address)` | `{pointer: int}` | Unsigned pointer-sized integer at an address. |
| `get_bookmarks()` | `{bookmarks: list[{index: int, address: int, description: str}]}` | All bookmarks defined in the database. |
| `add_bookmark(address, description)` | `None` | Add a bookmark at an address. |
| `delete_bookmark(index)` | `None` | Delete a bookmark by its index. |
| `set_name_at(address, name)` | `None` | Set the symbol name at an address. |
| `set_type_at(address, type_str)` | `None` | Set the type signature at an address. |
| `set_comment_at(address, comment)` | `None` | Set the comment at an address. |
| `set_repeatable_comment_at(address, comment)` | `None` | Set the repeatable comment at an address. |
| `set_local_variable_name(function_address, existing_name, new_name)` | `None` | Set the name of a local variable within a function. |
| `set_local_variable_type(function_address, existing_name, type_str)` | `None` | Set the type of a local variable within a function. |
